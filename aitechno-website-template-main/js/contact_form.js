...
function template_Contact_Form() {
    ...
    // Contact Form
    if ( jQuery('.template-contact-form:not(.is-init)').length ) {
        jQuery('.template-contact-form:not(.is-init)').each(function() {
            let $form = jQuery(this),
                $response = $form.find('.template-contact-form__response'),
                formData;
            ...
            $form.on('submit', function(e) {
                ...
                .done(function(response) {
                    ...
                    $response.empty().removeClass('template-alert-danger').addClass('template-alert-success').slideDown(200);
                ...
                .fail(function(data) {
                    $form.removeClass('is-busy');
                    $response.empty().removeClass('template-alert-success').addClass('template-alert-danger').slideDown(200);
