...
class template_Before_After {
	constructor( $obj = null ) {
        ...
        this.$el = {
            $wrap: $obj,
            $before : jQuery('<div class="template-before-after-img template-before-img"/>').appendTo($obj),
            $after : jQuery('<div class="template-before-after-img template-after-img-wrap"/>').appendTo($obj),
            $divider : jQuery('<div class="template-before-after-divider">\
        ...
        this.$el.$after.children('img').wrap('<div class="template-after-img"/>');
        this.$el.$after.children('.template-after-img').css('background-image', 'url('+ this.$el.$wrap.attr('data-img-after') +')');
	...
	layout() {
		this.offset = this.$el.$wrap.offset().left;
		this.size = this.$el.$wrap.width();
		this.$el.$after.children('.template-after-img').width( this.$el.$wrap.width() ).height( this.$el.$wrap.height() );
	}
	reset() {
		this.current = 50;
		this.target = 50;
...
const baw_axis = new template_PanAxis( 10 );
...
class template_Menu {
    constructor( _core = null ) {
        ...
        this.core.$dom.header.append('<span class="template-active-menu-ind"></span>');
        ...
        // Create Mobile Menu
        this.mobile_menu = new template_Mobile_Menu( this.$nav );
    }
    layout() {
        const _self = this;
        let active_pos = '-100%';
...
class template_Mobile_Menu {
    constructor( $obj )  {
        ...
        this.$nav = jQuery('<nav class="template-mobile-nav"/>').appendTo(this.$body);
        if ( this.$el.parents('header').hasClass('is-frosted') ) {
            this.$nav.addClass('is-frosted');
        }
        this.$menu = jQuery('<ul class="template-mobile-menu"/>').appendTo(this.$nav);
        this.$el.children('ul').children().each(function() {
            jQuery(this).clone().appendTo(_self.$menu);
        });
        this.$overlay = jQuery('<div class="template-mobile-menu-overlay"/>').appendTo(this.$body);
        ...
        // Events
        jQuery('.template-mobile-menu-toggler').on('click', function() {
...
class template_Infinite_List {
    constructor( $obj = null, cfg = {} )  {
        ...
        // Hover Event
        this.$el.on('mouseenter', '.template-infinite-list-item', function() {
            ...
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.template-infinite-list-preview').children('video')[0].play();
                $preview.children('video')[0].play();
            }
        }).on('mouseleave', '.template-infinite-list-item', function() {
            _self.$el.removeClass('is-hovererd');
            let $preview = _self.$bg_wrap.children('[data-id="'+ this.getAttribute('data-id') +'"]');
            //$preview.removeClass('is-active');
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.template-infinite-list-preview').children('video')[0].pause();
    ...
    init() {
        ...
        // DOM
        this.$list = this.$el.children('.template-infinite-list');
        this.$bg_wrap = jQuery('<div class="template-infinite-list-bgs"/>').appendTo(this.$el);
        this.$scroll = jQuery('<div class="template-infinite-scroll"/>').appendTo(this.$el);
        this.$cloned_list = jQuery('<div class="template-infinite-scroll-inner"/>').appendTo(this.$scroll);
    ...
    clone_items() {
        ...
        this.$list.children().each(function(i) {
            ...
            if ( _self.first_circle ) {
                let $preview = $cloned.children('.template-infinite-list-preview');
                ...
                if ( 'video' === type ) {
                    ...
                    <div class="templatel-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
                } else {
                    $preview.css('background-image', 'url('+ this_src + ')');
                    _self.$bg_wrap.append(`<div class="templatel-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`);
                }
            }
        });
...
class template_Marquee {
    constructor( $obj = null )  {
        ...
        this.$inner = this.$marquee.children('.template-marquee-inner');
        this.$content = this.$inner.children();
        this.width = this.$inner.find('.template-marquee-list').width();
    ...
    init() {
        this.$inner.wrapInner('<div class="template-marquee-inner-wrap"/>');
        this.$inner_cont = this.$inner.children('.template-marquee-inner-wrap');
...
class template_Masked {
    constructor( $obj = null, cfg = {} ) {
        ...
        // Options
        this.cfg = Object.assign({
            media_selector: '.template-masked-media',
            content_selector: '.template-masked-content',
            dbr: 24,
            ibr: 32
        }, cfg);
...
class template_LWP {
    constructor( $obj = null )  {
        ...
        this.$media = jQuery('<div class="template-lwp-media" data-appear="fade-right"/>').prependTo(this.$el);
        this.count = this.$list.children('.template-lwp-item').length;
        this.active = 0;
        this.prev = 1;
        

        this.$list.children('.template-lwp-item').each(function(i) {
