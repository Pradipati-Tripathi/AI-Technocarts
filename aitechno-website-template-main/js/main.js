...
const template_config = {
    ...
    // Cursor Following
    cursorFollow: '.template-socials-list a, .template-gallery-nav',

    // Links Exception
    linksException: [
        '.comment-reply-link',
        '.shadowcore-lightbox-link',
        '.template-lightbox-link',
        '.template-noanim-link'
    ]
}
...
jQuery(function ($) {
    ...
    const core = new ST_Core( template_config );
    ...
    function MainMenuInit() {
        template.menu = new template_Menu( core );
    }

    // --- PHOTOSWITE LIGHTBOX --- //
    function initPSWP() {
        if ( jQuery('.template-lightbox-link').length ) {
            if ( typeof PhotoSwipe === 'function' ) {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close template-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button template-pswp-nav template-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button template-pswp-nav template-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
                ...
                // Build PSWP Gallery
                jQuery('.template-lightbox-link').each(function() {
                    ...
                    if ( $this.attr('data-type') && $this.attr( 'data-type' ) !== 'image' ) {
                        ...
                        if ( 'video' === $this.attr('data-type') ) {
                            this_item.html = '\
                            <div class="template-pswp-media--video" data-src="' + $this.attr('href') + '">\
                            ...
                            this_item.html = '\
                            <div class="template-pswp-media--iframe" data-player-src="'+ this_link +'" data-src="' + $this.attr('href') + '" data-type="' + this_type + '">\
                ...
                core.$dom.body.on('click', '.template-pswp-image-wrap', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                ...
                jQuery(document).on('click', '.template-lightbox-link', function(e) {
                    e.preventDefault();
                    if (typeof PhotoSwipe === 'function') {
                        let $this = jQuery(this),
                        ...
                        template.pswp.lightbox.listen('resize', function() {
                            if ( jQuery(template.pswp.lightbox.container).find('.template-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                            if ( jQuery(template.pswp.lightbox.container).find('.template-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                        });
                        ...
                        template.pswp.lightbox.listen('beforeChange', function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.template-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--video').each(function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.template-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.template-pswp-media--iframe').each(function() {
                ...
                console.error('template Error: PSWP script not found.');
            }
        }
    }
    ...
    function HeroSections() {
        // Masked Blocks
        if ( jQuery('.template-masked-block').length ) {
            jQuery('.template-masked-block').each(function() {
                new template_Masked(this);
            });
        }
    }
    ...
    function BlocksHighlight() {
        if ( jQuery('.template-backlight').length ) {
            template.cursor_pos = {
                x: 0,
                y: 0
            }
    ...
    // --- MARQUEE ELEMENT --- //
    function Marquee() {
        if ( jQuery('.template-marquee').length ) {
            jQuery('.template-marquee').each(function() {
                new template_Marquee(this);
            });
        }
    }
        ...
        if ( jQuery('.swiper').length ) {
            ...
            const init_swiper = function() {
                ...
                jQuery('.swiper').each(function() {
                    ...
                    if ( core.$dom.body.hasClass('template-fullscreen-page') ) {
                        options.mousewheel = true;
                    }
                    // CAROUSEL
                    if ( $this.hasClass('template-carousel') ) {
                    ...
                    if ( $this.hasClass('template-slider') ) {
                        ...
                        // Slider Content
                        if ( $this.find('.template-slide-content').length ) {
                            ...
                            $this.children('.swiper-wrapper').children('.swiper-slide').each(function(i) {
                                ...
                                if ( $slide.children('.template-slide-content').length ) {
                                    const $slide_content = $slide.children('.template-slide-content');
                                    ...
                                    $media.wrap('<div class="template-slider-media"/>');
                                    ...
                                    $slide.children('.template-slider-media').css('clip-path', `url(#${(content_mask_id + i)})`);
                    ...
                    // Pagination
                    if ( $this.children('.template-dots').length ) {
                        options.pagination = {
                            el: '.template-dots',
                            dynamicBullets: true,
                    ...
                    // Slider Navigation
                    if ( $this.parent().children('.template-slider-nav').length ) {
                        let $this_nav = $this.parent().children('.template-slider-nav');
                        options.navigation = {
                            prevEl: $this_nav.children('.template-slider-prev')[0],
                            nextEl: $this_nav.children('.template-slider-next')[0],
                        }
            ...
            if ( typeof Swiper === 'function' ) {
                ...
                console.error('template Error: Swiper Library not found.')
            }
        }
    }
    ...
    function CircleLinks() {
        if ( jQuery('.template-circle-path-link:not(.is-init)').length ) {
            jQuery('.template-circle-path-link:not(.is-init)').each(function() {
                let $this = jQuery(this),
    ...
    function LinksEvents() {
        jQuery('.template-back-to-listing').on('click', function() {
            window.localStorage.setItem('template_return_from', this.getAttribute('data-id'));
        });
        if ( jQuery('.template-albums-listing').length && window.localStorage.getItem('template_return_from') != false ) {
            jQuery(window).trigger( 'template_set_album', window.localStorage.getItem('template_return_from') );
            window.localStorage.setItem('template_return_from', 0);
        } else {
            window.localStorage.setItem('template_return_from', 0);
        }
    }
    ...
    function InfiniteList() {
        if ( jQuery('.template-infinite-list-wrap:not(.is-init)').length ) {
            jQuery('.template-infinite-list-wrap:not(.is-init)').each(function() {
                this.classList.add('is-init');
                core.updateRequired.push( new template_Infinite_List(this) );
            })
        }
    }
    ...
    // --- COUNTERS --- //
    function Counters() {
        if ( jQuery('.template-counter:not(.is-init)').length ) {
            jQuery('.template-counter:not(.is-init)').each(function() {
                let $this = jQuery(this);
                this.classList.add('is-init');
                this.counter_count = function() {
                    let $counter = $this.children('.template-counter-number');
    ...
    function ListWithPreview() {
        if ( jQuery('.template-list-with-preview').length ) {
            jQuery('.template-list-with-preview').each(function() {
                new template_LWP(this);
            });
        }
    }
    ...
    function BeforeAfter() {
        if ( jQuery('.template-before-after:not(.is-init)').length ) {
            jQuery('.template-before-after:not(.is-init)').each(function() {
                jQuery(this).addClass('is-init');
                core.updateRequired.push( new template_Before_After(jQuery(this)) );
            });
        }
    }
    ...
    // --- TOGGLES --- //
    function Toggles() {
        if ( jQuery('.template-toggles-item:not(.is-init)').length ) {
            jQuery('.template-toggles-item:not(.is-init)').each(function() {
                let $this = jQuery(this);
                $this.addClass('is-init');
                $this.children('.template-toggles-item--title').append('<i class="template-icon template-icon-arrow-down"/>');
                $this.children('.template-toggles-item--content').slideUp(1);
                $this.on('click', function() {
                    jQuery(this).toggleClass('is-active');
                    $this.children('.template-toggles-item--content').slideToggle(300);
                });
                $this.on('click', '.template-toggles-item--content', function(e) {
                    e.stopPropagation();
                    e.preventDefault();
                });
            });
    ...
    function ScrollTrigger() {
        ...
        if ( jQuery('.template-inline-ribbon:not(.is-init)').length ) {
            jQuery('.template-inline-ribbon:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Sticky Elements
        if ( jQuery('.template-sticky-block:not(.is-init)').length ) {
            jQuery('.template-sticky-block:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Expand on Scroll
        if ( jQuery('.template-expand-on-scroll:not(.is-init)').length ) {
            jQuery('.template-expand-on-scroll:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Text Mask Fill
        if ( jQuery('.template-text-fill-mask:not(.is-init)').length ) {
            jQuery('.template-text-fill-mask:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Parallax Media
        if ( jQuery('.template-parallax-media:not(.is-init)').length ) {
            jQuery('.template-parallax-media:not(.is-init)').each(function() {
                let $this = jQuery(this);
    ...
    function ContactForm() {
        if ( jQuery('.template-contact-form').length ) {
            // The code related to the contact form is in the file: js/contact_form.js
            if ( typeof template_Contact_Form === 'function' ) {
                template_Contact_Form();
            } else {
                console.error('template Error: Contact form script not found.')
            }
        }
    }
    ...
    function InitAnimation() {
        ...
        template.checkURL = function( el ) {
            ...
            if ( this_href.indexOf('javascript') === 0 ) {
                ...
                jQuery(template_config.linksException).each(function() {
                    if ( $el.is(this) ) {
                        result = false;
                    }
                });
    ...
    function Layout() {
        ...
        if ( core.$dom.body.hasClass('template-fullscreen-page') ) {
            ...
            core.$dom.main.removeClass('template-min-content');
            ...
            if ( core.$dom.main.height() < minHeight ) {
                core.$dom.main.addClass('template-min-content');
    ...
    $(document).ready(function() {
        // General UI
        RightClickProtection( '.template-rcp-wrap' );
        MainMenuInit();
        CircleLinks();
